#!/bin/bash
# Submit an audio-driven Wan2.2 S2V request to the async video API.
#
# The script uploads a reference image and driving audio, polls the job until
# completion, and downloads the generated MP4 output.
...
INPUT_AUDIO="${INPUT_AUDIO:-$SCRIPT_DIR/../../../upstream_refs/Wan2.2/examples/talk.wav}"
OUTPUT_PATH="${OUTPUT_PATH:-wan22_s2v_output.mp4}"
...
create_response=$(curl -sS -X POST "${BASE_URL}/v1/videos" \
  ...
  -F "input_audio_reference=@${INPUT_AUDIO}" \
  -F "width=832" \
  -F "height=448" \
  -F "num_frames=17" \
