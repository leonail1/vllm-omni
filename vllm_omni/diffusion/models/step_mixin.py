...
class DiffusionV2AtomDefaultsMixin:
    """Default hooks shared by step diffusion pipelines."""
    ...
    # TODO: build varlen attention metadata here when step batching starts
    # using packed varlen attention.
...
class DiffusionV2CFGStepMixin(DiffusionV2AtomDefaultsMixin):
    ...
    def denoise_step(
        ...
        **kwargs: Any,
        ...
        # The current runner passes its state list for model-specific step
        # implementations. Generic CFG pipelines read state through InputBatch.
        del kwargs
        if self.interrupt:
            return None
