...
class DiffusionEngine:
    ...
    def __init__(
        ...
        self.stage_split = bool(getattr(od_config, "stage_split", False))
        self.stage_role = getattr(od_config, "stage_role", "all")
        self._validate_stage_step_config()
        self.stepwise_execution = self.stage_split and self.stage_role == "dit"
        if getattr(self.od_config, "streaming_output", False) and not (self.stage_split and self.step_execution):
            raise ValueError("streaming_output requires stage_split=True and step_execution=True.")

        executor_class = DiffusionExecutor.get_class(od_config)
        self.executor = executor_class(od_config)
        self.scheduler: SchedulerInterface = scheduler or (
            StepScheduler() if self.stepwise_execution else RequestScheduler()
        )
        self.scheduler.initialize(od_config)
        if self.scheduler.max_num_running_reqs > 1 and not self.stepwise_execution and not self.stage_split:
        ...
        if self.stepwise_execution:
            self.execute_fn = self.executor.execute_step
        elif not self.stage_split:
            self.execute_fn = self.executor.execute_request
        else:
            self.execute_fn = self._reject_split_role_scheduler_execution

        if self._should_skip_dummy_run():
            logger.info(
                "Skipping dummy warmup run for diffusion stage_split role=%s",
                self.od_config.stage_role,
            )
        else:
            try:
                self._dummy_run()
    ...
    def _validate_stage_step_config(self) -> None:
        # Stage split and DiT step execution are currently one deployment mode:
        # both flags must be enabled together for split roles, or disabled for the full pipeline.
        if self.stage_split != self.step_execution:
            raise ValueError("stage_split and step_execution must be enabled or disabled together.")
        if self.stage_split:
            if self.stage_role not in ("encode", "dit", "decode"):
                raise ValueError("stage_split=True requires stage_role to be one of 'encode', 'dit', or 'decode'.")
        elif self.stage_role != "all":
            raise ValueError("stage_role must be 'all' when stage_split=False.")

    def _reject_split_role_scheduler_execution(self, scheduler_output: object) -> BaseRunnerOutput:
        raise RuntimeError(
            f"stage_role={self.stage_role!r} is request/transport driven and cannot "
            "be executed through the scheduler loop."
        )
    ...
    async def async_add_stage_transport_and_wait_for_response(
        ...
        """Schedule an encode->dit transport through the step scheduler.

        This is the DiT-role equivalent of ``async_add_req_and_wait_for_response``:
        the payload enters the normal StepScheduler/batched runner loop, and the
        terminal runner output carries the dit->decode payload.
        """
        if not (self.stage_split and self.step_execution and self.stage_role == "dit"):
            raise RuntimeError(
                "stage DiT transport requires stage_split=True, step_execution=True, "
                "and stage_role='dit'."
            )
        await self._check_and_start_background_loop()
        # Wrap the transport as a request so existing scheduling/lifecycle code is reused.
