...
"""Build a per-block chunk manifest; digest payload matches original stage1."""
...
def _full_chunk_numel(dtype: torch.dtype, weight_shard_size: int, chunk_size_bytes: int, alignment_bytes: int) -> tuple[int, int]:
...
def build_part_manifest(
    tensor_specs: Sequence[TensorSpec], *, block_id: int, part_id: str, weight_shard_size: int,
    weight_shard_rank: int, chunk_size_bytes: int, alignment_bytes: int = 256,
    ...
    for dtype, dtype_specs in grouped.items():
        ...
        for name, tensor, is_buffer in dtype_specs:
            ...
            tensor_metas.append(TensorMeta(name, offset, numel, tuple(tensor.shape), is_buffer, stride=tuple(tensor.stride())))
            offset += numel
        total_numel, chunks, cpu_offset = offset, [], 0
        if layout is WeightLayout.CHUNK_MAJOR:
            full_chunk_numel, alignment_numel = _full_chunk_numel(dtype, weight_shard_size, chunk_size_bytes, alignment_bytes)
        ...
        dtype_manifests.append(DTypeManifest(
            dtype, tuple(tensor_metas), tuple(chunks), total_numel, padded_total, cpu_offset,
            max((c.local_numel for c in chunks), default=0), alignment_numel,
        ))
    payload = {
        "block_id": block_id, "part_id": part_id, "weight_shard_size": weight_shard_size,
        "chunk_size_bytes": chunk_size_bytes, "alignment_bytes": alignment_bytes, "layout": layout.value,
        "dtypes": [{
            "dtype": str(dm.dtype),
            "tensors": [{"name": t.name, "offset": t.offset, "numel": t.numel, "shape": t.shape,
                         "is_buffer": t.is_buffer, "owner": "chunked_fs_offload", "stride": t.stride} for t in dm.tensors],
            "chunks": [c.__dict__ for c in dm.chunks],
        } for dm in dtype_manifests],
    }
    digest = hashlib.sha256(json.dumps(payload, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
    return PartManifest(
        block_id, part_id, weight_shard_size, weight_shard_rank, chunk_size_bytes,
        alignment_bytes, layout, tuple(dtype_manifests), digest,
    )
