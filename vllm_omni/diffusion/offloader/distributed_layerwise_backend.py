...
class DistributedLayerwiseOffloadBackend(OffloadBackend):
    ...
    def _init_weight_shard_group(self) -> None:
        ...
        if not self.config.use_hsdp:
            ...
            # Weight AllGathers run from the prefetch worker while SP
            # activation collectives run from the compute thread. Give DLO a
            # companion communicator so their ordering cannot interleave.
