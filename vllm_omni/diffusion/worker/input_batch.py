...
def _state_latents(state: DiffusionRequestState) -> torch.Tensor:
    if state.latents is None:
        raise ValueError(f"Request {state.req_id} has no latents.")
    ...
    *,
    ...
    for state, (embeds, mask), lengths in zip(states, fields, seq_lens_by_state, strict=True):
        ...
        if mask is None and max(lengths) != target_len:
            raise ValueError(
                f"Variable-length {embeds_attr} in batch but {mask_attr} is None for request {state.req_id}."
            )
...
def _freeze_img_shape(value):
    if torch.is_tensor(value):
        value = value.detach().cpu().tolist()
    if isinstance(value, (list, tuple)):
        return tuple(_freeze_img_shape(item) for item in value)
    return int(value)


def _img_shapes_key(state: DiffusionRequestState):
    img_shapes = getattr(state, "img_shapes", None)
    if not img_shapes:
        return None
    return _freeze_img_shape(img_shapes)
...
def _needs_dynamic_batch(
    ...
    *,
    ...
    if negative_lens is not None and len(set(negative_lens)) > 1:
        dynamic = True
    if len({_img_shapes_key(state) for state in states}) > 1:
        dynamic = True
...
class InputBatch:
    ...
    def make_batch(
        ...
        request_ids = [state.req_id for state in selected]
