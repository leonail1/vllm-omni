...
from vllm_omni.diffusion.offloader.submodule.common.bucket_weight_layout import BucketWeightLayout
...
def test_qkv_packing_roundtrip(buckets, shape):
    layout = BucketWeightLayout(HeadBucketPlan(32, 32, 2, 4, buckets))
    value = torch.arange(torch.tensor(shape).prod().item(), dtype=torch.float32).reshape(shape)
    torch.testing.assert_close(layout.unpack_qkv(layout.pack_qkv(value)), value, rtol=0, atol=0)
...
def test_adapter_disable_restores_forward_and_weights_for_reenable(adapter_class):
    ...
    for _ in range(2):
        ...
        assert not hasattr(module, "_h3_bucket_adapter")
