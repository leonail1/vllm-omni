...
def _state(
    ...
    *,
    ...
    state = DiffusionRequestState(req_id=request_id, sampling=sampling, prompts=["prompt"])
...
def test_qwen_image_same_token_count_different_shape_uses_dynamic_view() -> None:
    input_batch = InputBatch.make_batch(
        [
            _state("req-1", image_len=8, txt_len=5, img_shape=[(1, 2, 4)]),
            _state("req-2", image_len=8, txt_len=5, img_shape=[(1, 4, 2)]),
        ],
        allow_dynamic=True,
    )
...
def test_qwen_image_prompt_padding_can_be_trimmed_to_real_mask_length() -> None:
    state = _state("req-1", image_len=4, txt_len=20)
    state.prompt_embeds_mask[:, 17:] = False

    input_batch = InputBatch.make_batch([state], allow_dynamic=False)

    assert input_batch.is_dynamic is False
    assert input_batch.prompt_embeds.shape[1] == 17
    assert input_batch.prompt_embeds_mask is not None
    assert input_batch.prompt_embeds_mask.shape[1] == 17
    assert input_batch.txt_seq_lens == [17]
...
def test_qwen_image_dynamic_batch_fails_fast_with_cache_backend() -> None:
    ...
    assert runner._supports_qwen_image_dynamic_step_batching() is False

    input_batch = InputBatch.make_batch(
        [_state("req-1", image_len=4, txt_len=5)],
        allow_dynamic=runner._supports_qwen_image_dynamic_step_batching(),
    )
    assert input_batch.is_dynamic is False
...
def test_qwen_image_dynamic_batch_requires_explicit_additional_config() -> None:
    runner = object.__new__(DiffusionModelRunner)
    runner.pipeline = type(
        "QwenImagePipeline",
        (),
...
def test_qwen_image_dynamic_validation_allows_missing_parallel_config() -> None:
    runner = object.__new__(DiffusionModelRunner)
    runner.pipeline = type(
        "QwenImagePipeline",
        (),
...
def test_qwen_image_dynamic_batch_can_be_disabled_by_additional_config() -> None:
    runner = object.__new__(DiffusionModelRunner)
    runner.pipeline = type(
        "QwenImagePipeline",
        (),
    ...
    runner.od_config = SimpleNamespace(
        step_execution=True,
        max_num_seqs=2,
        additional_config={"diffusion_dynamic_step_batching_enabled": False},
    )
...
def test_qwen_image_dynamic_stepwise_updates_request_local_latents() -> None:
    ...
    scheduler_output = DiffusionSchedulerOutput(
        step_id=0,
        scheduled_new_reqs=[NewRequestData(sched_req_id=req.request_id, req=req) for req in requests],
    ...
    second = result.get_req_output("req-2")
